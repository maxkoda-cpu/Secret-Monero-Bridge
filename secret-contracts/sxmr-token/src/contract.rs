...
use crate::state::{
    get_receiver_hash, get_transfers, get_txs, read_allowance, read_viewing_key,
    remove_receiver_hash, set_receiver_hash, store_burn, store_deposit, store_mint, store_redeem,
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    let response = match msg {
        ...
        HandleMsg::DeRegisterReceive { .. } => try_deregister_receive(deps, env),
...
fn try_deregister_receive<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    env: Env,
) -> StdResult<HandleResponse> {
    remove_receiver_hash(&mut deps.storage, &env.message.sender);

    Ok(HandleResponse {
        messages: vec![],
        log: vec![log("deregister_status", "success")],
        data: Some(to_binary(&HandleAnswer::DeRegisterReceive {
            status: Success,
        })?),
    })
}
